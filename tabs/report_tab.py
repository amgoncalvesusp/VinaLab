...
class ReportTab(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.form_labels = {key: QLabel() for key in ("best_affinity", "mean_affinity", "ligands_docked", "poses_per_ligand")}
    ...
    def generate_pdf_report(self) -> None:
        ...
        if not self.results:
            QMessageBox.warning(self, I18n.get("warning_title", self.lang), I18n.get("no_results", self.lang))
            return
        file_name, _ = QFileDialog.getSaveFileName(self, I18n.get("generate_pdf", self.lang), "", "Arquivos PDF (*.pdf)")
        if not file_name:
            return
        ...
        story = [
            Paragraph("Relatório completo de docking VinaLab", styles["Title"]),
            Paragraph(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), styles["Normal"]),
            Spacer(1, 12),
            Paragraph("1. Parâmetros de docking", styles["Heading2"]),
        ]
        for key, value in self.parameters.items():
            story.append(Paragraph(f"{key}: {value}", styles["Normal"]))
        story.extend([Spacer(1, 12), Paragraph("2. Tabela de resultados", styles["Heading2"])])
        story.append(self._dynamic_results_table())
        story.extend([Spacer(1, 12), Paragraph("3. Ranking de consenso", styles["Heading2"])])
        story.append(self._consensus_table())
        story.extend([Spacer(1, 12), Paragraph("4. Resumo de clusters", styles["Heading2"])])
        story.append(self._cluster_summary_table())
        story.extend([Spacer(1, 12), Paragraph("5. Resumo de interações", styles["Heading2"])])
        story.append(self._interaction_summary_table())
        story.extend([Spacer(1, 12), Paragraph("6. Erros e avisos", styles["Heading2"])])
        story.append(self._warnings_table())
        if self.chart_path and self.chart_path.exists():
            story.extend([Spacer(1, 12), Paragraph(I18n.get("affinity_chart", self.lang), styles["Heading2"])])
    ...
    def open_output_folder(self) -> None:
        """Open the selected output folder using Windows shell integration."""
        if not self.output_directory or not self.output_directory.exists():
            QMessageBox.warning(self, I18n.get("warning_title", self.lang), I18n.get("missing_output_folder", self.lang))
            return
    ...
    def _refresh_summary(self) -> None:
        ...
        self.poses_label.setText(", ".join(f"{ligand}: {count}" for ligand, count in counts.items()))

    def _dynamic_results_table(self) -> Table:
        """Create top-results report table with dynamic scoring columns."""
        scoring_labels = sorted({row.get("scoring_function", row.get("scoring_key", "")) for row in self.results})
        data = [["Ligante", "Pose", "Score de docking", *scoring_labels]]
        grouped: dict[str, list[dict]] = {}
        for row in self.results:
            grouped.setdefault(row["ligand_name"], []).append(row)
        for ligand, rows in sorted(grouped.items()):
            for row in sorted(rows, key=lambda item: float(item.get("affinity", 0.0)))[:10]:
                ...
                data.append([
                    ...
                    *[f"{float(row.get('affinity', 0.0)):.3f}" if scoring == label else "" for scoring in scoring_labels],
                ])
    ...
    def _consensus_table(self) -> Table | Paragraph:
        ...
        if provider is None or len(provider._scoring_headers()) < 2:
            return Paragraph("O consenso requer pelo menos duas funções de pontuação.", getSampleStyleSheet()["Normal"])
        rows = provider._build_consensus_rows(provider._scoring_headers())[:20]
        data = [["Ligante", "Pose", "Rank médio", "Borda", "Z-score", "Divergência"]]
        for row in rows:
            data.append([
                row["ligand"],
                str(row["pose_rank"]),
    ...
    def _cluster_summary_table(self) -> Table | Paragraph:
        ...
        if provider is None or not getattr(provider, "current_clusters", None):
            return Paragraph("Nenhum resumo de clusters disponível ainda. Abra a aba Clusters após o docking para preencher esta seção.", getSampleStyleSheet()["Normal"])
        data = [["Ligante", "ID do cluster", "Tamanho", "Pose representante", "Melhor score"]]
        for cluster in provider.current_clusters:
            ...
            data.append([
                ...
                str(cluster["size"]),
                str(representative.get("pose_rank", representative.get("mode", ""))),
    ...
    def _interaction_summary_table(self) -> Table | Paragraph:
        ...
        provider = self.analysis_provider
        if provider is None:
            return Paragraph("Provedor de interações indisponível.", getSampleStyleSheet()["Normal"])
        try:
            from tabs.results_tab import prepare_pose_view_files
        except ImportError:
            return Paragraph("Auxiliares de interação indisponíveis.", getSampleStyleSheet()["Normal"])
        data = [["Ligante", "Principais resíduos por frequência de contato"]]
        grouped: dict[str, list[dict]] = {}
        for row in self.results:
            grouped.setdefault(row["ligand_name"], []).append(row)
        for ligand, rows in sorted(grouped.items()):
            ...
            try:
                ...
                interactions = provider._compute_pose_interactions(best, receptor_pdb, pose_pdb)
                residues = Counter(f"{item['residue_name']}{item['residue_number']}" for item in interactions)
                summary = ", ".join(f"{residue} ({count})" for residue, count in residues.most_common(5)) or "Nenhum contato detectado"
            except Exception as exc:  # noqa: BLE001 - report should still be generated
                summary = f"Ignorado: {exc}"
            data.append([ligand, summary])
        return self._styled_table(data)

    def _warnings_table(self) -> Table:
        """Create warnings/errors report table."""
        warnings = [row.get("scoring_error", "") for row in self.results if row.get("scoring_error")]
        data = [["Tipo", "Mensagem"]]
        if warnings:
            for message in warnings:
                data.append(["Pontuação", message])
        else:
            data.append(["Nenhum", "Nenhum erro de pontuação registrado nos resultados atuais."])
        return self._styled_table(data)
    ...
    def _draw_footer(self, canvas, doc) -> None:
        ...
        canvas.saveState()
        footer = I18n.get("pdf_footer", self.lang).format(date=datetime.now().strftime("%Y-%m-%d"))
